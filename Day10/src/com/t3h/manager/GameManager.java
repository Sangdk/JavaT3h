...
public class GameManager {
    ...
    public void initBoss() {
        ...
        Boss b = new Boss(0, 0);
        arrBoss.add(b);
        Boss b1 = new Boss(BaseFrame.W_FRAME / 2, 0);
        arrBoss.add(b1);
        Boss b2 = new Boss(BaseFrame.W_FRAME - 100, 0);
        arrBoss.add(b2);
    }
    ...
    private boolean handleBullet(ArrayList<Bullet> arrBullet) {
        for (int i = arrBullet.size() - 1; i >= 0; i--) {
            ...
            for (int j = 0; j < arrMap.size(); j++) {
                Rectangle rect = arrBullet.get(i).getRect().intersection(arrMap.get(j).getRect());
